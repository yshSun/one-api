...
type Token struct {
	...
	RateLimitRpm   int     `json:"rate_limit_rpm" gorm:"default:0"`    // rate limit per minute, 0 means no limit
}
...
func (t *Token) Update() error {
	var err error
	err = DB.Model(t).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota", "models", "subnet", "rate_limit_rpm").Updates(t).Error
	return err
}
